/*

SET

Abstract data type
Stores unique values in no particular order
Lacks a mechanism for retrieving elements
Your set should be able to store any JavaScript primitive

*** Operations:

mySet.count()
=> returns integer value for # of values present in set

mySet.add(value)
=> returns the set's object

mySet.delete(value)
=> returns true if value was removed & false if value was not removed

mySet.has(value)
=> returns true/false

mySet.forEach(callbackFn)
=> no return value, calls callbackFn once for each value in the set

Note: ES6 has a Set data structure as part of the core language.

*** Additional Exercises:

Modify your set to take a max capacity and return a string if you try
to add an element when there's no more room 

mySet.add(value)
=> "Max capacity already reached. Remove element before adding a new one."

Make your set able to take objects, arrays, and functions as values in
addition to just primitives.

 */
function Set(capacity) {
  this._capacity = capacity;
  this._storage = {};
  this._count = 0;
}


// O(1)
Set.prototype.count = function() {
  return this._count;
};


// O(1)
Set.prototype.has = function(value) {
  return !!this._storage[value];
};


// O(1)
Set.prototype.add = function(value) {
  if (this.has(value)) {
    return 'Set already has value';
  }
  else if (this._count < this._capacity) {
    this._storage[value] = true;
    this._count++;
    return this;
  }
  return 'Max capacity already reached. Remove element before adding a new one.'
};


// O(1)
Set.prototype.delete = function(value) {
  if (this._storage[value]) {
    delete this._storage[value];
    this._count--;
    return true;
  }
  return false;
};


// O(n)
Set.prototype.forEach = function(callback) {
  var values = this._storage;
  for (var key in values) {
    callback(key);
  }
};


/*
*** Exercises:

1* Write these set theory operations:

mySet.union(otherSet)
=> returns mySet with added values from otherSet

Add any values from otherSet into mySet that are not already there
ex: {1,2,3} union {2,3,4} => {1,2,3,4}


mySet.intersection(otherSet)
=> returns mySet with values removed that are not in otherSet

Remove values from mySet that are not in otherSet
ex: {1,2,3} intersection {2,3,4} => {2,3}


mySet.difference(otherSet)
=> returns mySet with values removed that are in otherSet

Remove values from mySet that are in otherSet
ex: {1,2,3} difference {2,3,4} => {1}


mySet.hasSubset(otherSet)
=> returns true/false depending on if otherSet is a subset of mySet
ex: {1,2,3} hasSubset {2,3,4} => false
ex: {1,2,3} hasSubset {2,3} => true


2* Using a set, create a whitelist filter:
      Given a list of whitelist items and a collection to be filtered,
      return an array with only the items from the collection that are
      on the whitelist:

whitelistFilter(collection <array>, whitelist <array>)
=> filtered collection <array> with only items from white list

3* Create a blacklist filter

* exercises adapted from Algorithms, 4th Edition
  by Robert Sedgewick and Kevin Wayne

 */