...
INSERTION SORT
*** Description
  Iterate over array & grow sorted array behind current element.
  For each spot, compare element value with previous elements &
swap spots if previous element is larger.

Example: [ 3 4 5|1 2 6 ]  --> [sorted|unsorted]
...
Properties: O(1) extra space

Time complexity:
- worst: O(n^2) comparisons and swaps
- best: O(n) when nearly sorted
- stable & adaptive --> O(n) time when nearly sorted

Use cases:
  When the data is nearly sorted (since it's adaptive) or when 
the problem size is small (because it has low memory overhead)
*/
